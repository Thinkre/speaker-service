"""Local CamPlus speaker embedding engine via funasr.AutoModel."""
...
def _resolve_model_id() -> str:
    import os
    return os.environ.get("CAMPPLUS_MODEL_PATH") or _MODEL_ID_DEFAULT


class CamPlusEngine:
    """Local CamPlus speaker embedding using funasr.AutoModel (lazy-loaded)."""
    ...
    def _load(self) -> None:
        ...
        with self._load_lock:
            ...
            logger.info("CamPlus loaded: %s", model_id)
    ...
    def _extract_sync(self, pcm_bytes: bytes) -> np.ndarray | None:
        ...
        try:
            ...
            logger.warning("CamPlus extract error: %s", exc)
            return None
        if not result:
            return None
